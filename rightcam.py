...
def right_rail_edge(
    image,
    rail_mask_right_inner,
    rail_mask_right_outer,
    right_inner_buffer,
    right_outer_buffer,
    disp_mask=False,
):
    ...
    edges = cv2.Canny(image=image, threshold1=threshold / 2, threshold2=threshold)
    ...
    strlist = []
    for x in l:
        angle = np.arctan2(x[0][3] - x[0][1], x[0][2] - x[0][0]) * 180.0 / np.pi
    ...
    minR = rail_mask_right_outer - right_outer_buffer
    temp = []
    for x in strlist:
        if x[0][0] > rail_mask_right_inner + right_inner_buffer:
            if x[0][0] < minR:
                minR = x[0][0]
                temp = x
    strlist = []
